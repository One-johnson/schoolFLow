...
export default function PaymentPage(): React.JSX.Element {
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      toast.success('Payment proof submitted successfully! Awaiting admin approval.');
      ...
      <div className="max-w-2xl mx-auto">
        <Card className="hover:shadow-lg transition-shadow">
          <CardHeader>
            ...
            <CardDescription>
              Your payment proof is currently being reviewed by the administrator
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="grid gap-2">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{pendingApproval.planName}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Amount:</span>
                  <span className="font-medium">₵{pendingApproval.totalAmount.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
      ...
      {pendingSubscription && (
        <Card className="bg-primary/5 border-primary hover:shadow-lg transition-all hover:border-primary/50">
          <CardHeader>
            ...
            <CardDescription>Review your subscription before payment</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-3">
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground">Plan:</span>
                <Badge variant="outline" className="font-medium">{pendingSubscription.planName}</Badge>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Students:</span>
                <span className="font-medium">{pendingSubscription.studentsCount}</span>
              </div>
              <div className="flex justify-between text-lg font-bold pt-2 border-t">
                <span>Total Amount:</span>
                <span className="text-primary">₵{pendingSubscription.totalAmount.toFixed(2)}</span>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <ul className="space-y-2 text-sm">
                <li className="flex items-start gap-2">
                  ...
                  <span><strong>MTN Mobile Money:</strong> 0712345678</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-primary">•</span>
                  <span><strong>Airtel Money:</strong> 0787654321</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-primary">•</span>
            ...
            <div className="space-y-2">
              ...
              <ul className="space-y-2 text-sm">
                <li className="flex items-start gap-2">
                  <span className="text-primary">•</span>
                  <span><strong>Bank:</strong> Ghana Commercial Bank</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-primary">•</span>
                  <span><strong>Account Name:</strong> SchoolFlow Systems</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-primary">•</span>
                  <span><strong>Account Number:</strong> 1234567890</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-primary">•</span>
                  <span><strong>Branch:</strong> Accra Main Branch</span>
                </li>
              </ul>
            </div>
          </div>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <RadioGroup value={paymentMethod} onValueChange={(value) => setPaymentMethod(value as 'mobile_money' | 'bank_transfer')}>
                <div className="grid md:grid-cols-2 gap-3">
                  <div className={cn(
                    "flex items-center space-x-3 p-4 rounded-lg border-2 cursor-pointer transition-all",
                    paymentMethod === 'mobile_money' ? 'border-primary bg-primary/5' : 'border-muted hover:border-primary/50'
                  )}>
                    <RadioGroupItem value="mobile_money" id="mobile_money" />
                    <Label htmlFor="mobile_money" className="font-normal flex items-center gap-2 cursor-pointer">
                  ...
                  <div className={cn(
                    "flex items-center space-x-3 p-4 rounded-lg border-2 cursor-pointer transition-all",
                    paymentMethod === 'bank_transfer' ? 'border-primary bg-primary/5' : 'border-muted hover:border-primary/50'
                  )}>
