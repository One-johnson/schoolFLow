...
export default function ParentLoginPage(): React.JSX.Element {
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      if (result.success) {
        ...
        router.push(result.redirectTo || '/parent');
      } else {
    ...
    <div className="min-h-screen flex flex-col bg-muted/30">
      <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 h-16 bg-background/80 backdrop-blur-lg border-b border-border">
        ...
        <div className="flex items-center gap-4">
          <Link href="/parent/register" className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors">
            Register
          </Link>
