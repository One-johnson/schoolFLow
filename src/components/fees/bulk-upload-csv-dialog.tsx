          ...
          (results.data as Partial<Record<string, string>>[]).forEach(
            (row, index) => {
              try {
                ...
                const amountDue = parseFloat(String(row.amountDue));
                const amountPaid = parseFloat(String(row.amountPaid));
                ...
                if (!validMethods.includes(paymentMethod)) {
                  validationErrors.push(
                    `Row ${index + 1}: Invalid payment method`,
                  );
                  return;
                }
                ...
                parsedData.push({
                  ...
                  paymentDate:
                    row.paymentDate || new Date().toISOString().split("T")[0],
                  paidBy: row.paidBy,
                  notes: row.notes,
                });
              } catch (error) {
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <p className="font-medium">
                  Preview: {csvData.length} records ready for import
                </p>
              </div>
                  ...
                  <TableBody>
                    {csvData.slice(0, 10).map((payment, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          GHS {payment.amountDue.toFixed(2)}
                        </TableCell>
                        <TableCell>
                          GHS {payment.amountPaid.toFixed(2)}
                        </TableCell>
