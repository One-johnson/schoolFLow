...
import { PrintLayoutSettings, type PrintLayoutOptions } from './print-layout-settings';
...
export function ReportCardSheet({ open, onOpenChange, reportCardId }: ReportCardSheetProps) {
  ...
  const academicYear = useQuery(
    ...
    reportCard?.academicYearId ? { yearId: reportCard.academicYearId as Id<'academicYears'> } : 'skip'
  );
  ...
  const handleDownload = (options?: PrintLayoutOptions): void => {
    ...
    const enrichedReport = {
      ...reportCard,
      academicYearName: academicYear?.yearName || reportCard.academicYearName || 'N/A',
      ...
      <SheetContent side="right" className="w-full sm:max-w-4xl overflow-y-auto">
        ...
        <div className="mt-6 bg-white text-black p-6 border-2 border-black">
                ...
                {photoUrl && (
                  <Avatar className="h-24 w-24 border-2 border-black">
                    ...
                    <AvatarFallback className="text-2xl">
                      {reportCard.studentName.split(' ').map((n: string) => n[0]).join('')}
                    </AvatarFallback>
                  </Avatar>
                )}
              </div>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm mb-6">
            ...
            <div>
              <p><span className="font-semibold">Year:</span> {academicYear?.yearName || reportCard.academicYearName || 'N/A'}</p>
              <p><span className="font-semibold">Term:</span> {term?.termName || reportCard.termName || 'N/A'}</p>
            </div>
          </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-b-2 border-black bg-gray-100 hover:bg-gray-100">
                  <TableHead className="border-r border-black text-black font-bold text-left">SUBJECT</TableHead>
                  <TableHead className="border-r border-black text-black font-bold text-center">CLASS SCORE (%)</TableHead>
                  <TableHead className="border-r border-black text-black font-bold text-center">EXAMS SCORE (%)</TableHead>
                  <TableHead className="border-r border-black text-black font-bold text-center">TOTAL SCORE (%)</TableHead>
                  <TableHead className="border-r border-black text-black font-bold text-center">POSITION</TableHead>
                  <TableHead className="border-r border-black text-black font-bold text-center">GRADE</TableHead>
                  <TableHead className="text-black font-bold text-center">REMARKS</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {subjects.map((subject: {
                  ...
                  <TableRow key={index} className="border-b border-black hover:bg-gray-50">
                    <TableCell className="border-r border-black text-left">{subject.subjectName}</TableCell>
                    <TableCell className="border-r border-black text-center">{subject.classScore}</TableCell>
                    <TableCell className="border-r border-black text-center">{subject.examScore}</TableCell>
                    <TableCell className="border-r border-black text-center font-semibold">{subject.totalScore}</TableCell>
                    <TableCell className="border-r border-black text-center">{subject.position}</TableCell>
                    <TableCell className="border-r border-black text-center font-semibold">{subject.grade}</TableCell>
                    <TableCell className="text-center">{subject.remarks}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm mb-6">
            ...
            <div className="space-y-2">
              {reportCard.conduct && (
                <p><span className="font-semibold">Conduct:</span> {reportCard.conduct}</p>
              )}
              {reportCard.attitude && (
                <p><span className="font-semibold">Attitude:</span> {reportCard.attitude}</p>
              )}
              {reportCard.interest && (
                <p><span className="font-semibold">Interest:</span> {reportCard.interest}</p>
              )}
            </div>
          </div>
