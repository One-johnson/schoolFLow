...
export const generateReportCards = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (students.length === 0) {
      throw new Error(
        "No students found in the selected class (excluding graduated students)",
      );
    }
    ...
    for (const student of students) {
      try {
        ...
        if (marks.length === 0) {
          errors.push(
            `${student.firstName} ${student.lastName}: No marks found`,
          );
          continue;
        }
        ...
        if (existing) {
          ...
          // Create new report
          const reportId: Id<"reportCards"> = await ctx.db.insert(
            "reportCards",
            {
        ...
        errors.push(
          `${student.firstName} ${student.lastName}: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    }
    ...
    if (reportIds.length === 0) {
      throw new Error(
        `Failed to generate any report cards. Errors: ${errors.join("; ")}`,
      );
    }
